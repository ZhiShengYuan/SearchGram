...
- `auth.use_jwt`: Enable JWT authentication (default: `false`, **MUST be `true` for production**)
- `auth.public_key_path`: Path to Ed25519 public key PEM file (for token verification)
- `auth.private_key_path`: Path to Ed25519 private key PEM file (for token signing)
- `auth.public_key_inline`: Inline public key as string or array (alternative to file path)
- `auth.private_key_inline`: Inline private key as string or array (alternative to file path)
- `auth.issuer`: Token issuer identifier (e.g., "bot", "userbot", "search")
- `auth.audience`: Expected audience for incoming tokens (e.g., "search", "userbot")
- `auth.token_ttl`: Token time-to-live in seconds (default: `300` = 5 minutes)

**Authentication Architecture:**
- All HTTP APIs use Ed25519-signed JWT tokens for authentication
- Each service has an issuer identity: `bot`, `userbot`, `search`
- Tokens are generated by the calling service and verified by the receiving service
- **Go Search Service** (`/api/v1/*`): Accepts tokens from `bot`, `userbot`, `search`
- **Python Userbot Sync API** (`/api/v1/sync/*`): Accepts tokens from `bot`
- Both services share the same Ed25519 key pair for signing/verification
...
- Userbot sync API endpoint: `services.userbot.base_url` (default: `http://127.0.0.1:8082`)
...
- Authentication: **Requires JWT** - configure `auth` section (see above)
